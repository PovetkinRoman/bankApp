...
- **ConfigMaps**: Конфигурация приложений в Kubernetes
...
### Проблемы с ConfigMaps

Проверьте, что ConfigMaps созданы:

```bash
kubectl get configmaps -n bankapp
```

Проверьте содержимое ConfigMap:

```bash
kubectl describe configmap <configmap-name> -n bankapp
```
...
- Service Discovery работает через Kubernetes DNS
- Конфигурация через Kubernetes ConfigMaps и Secrets
